...
import Distribution.Compat.Prelude (readMaybe, sort, sortBy)
...
main :: IO ()
main = do
  let digits = map (\word -> convertDigitToInt $ concatHeadAndLastDigit (map convertFullDigitsToDigits (pickDigits $ sortCapturedDigits (captureDigits word)))) input
  let amount = sum digits
  _ <- print amount
  return ()

occursAt :: ([a] -> [a] -> Bool) -> [a] -> [a] -> Int -> [([a], Int)] -> [([a], Int)]
occursAt predicate expect list start occurrences = case (expect, list) of
  ([], _) -> occurrences
  (_, []) -> occurrences
  (expect, list)
    | start + 1 > length list -> occurrences
    | predicate expect (take (length expect) (drop start list)) -> occursAt predicate expect list (start + 1) (occurrences ++ [(take (length expect) (drop start list), start)])
    | otherwise -> occursAt predicate expect list (start + 1) occurrences

captureDigits :: String -> [(String, Int)]
captureDigits word = foldl (\occurrences digit -> occurrences ++ occursAt (==) digit word 0 []) [] allowedDigits

sortCapturedDigits :: [(String, Int)] -> [(String, Int)]
sortCapturedDigits = sortBy (\(_, offsetA) (_, offsetB) -> compare offsetA offsetB)

pickDigits :: [(b1, b2)] -> [b1]
pickDigits = map fst

convertFullDigitsToDigits :: String -> String
convertFullDigitsToDigits digit = fromMaybe digit (M.lookup digit fullDigitsToDigits)

concatHeadAndLastDigit :: [String] -> String
concatHeadAndLastDigit digits = head digits <> last digits

convertDigitToInt :: String -> Int
convertDigitToInt strNumber = read strNumber :: Int